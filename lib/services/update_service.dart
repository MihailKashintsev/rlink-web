...
class UpdateInfo {
  ...
  const UpdateInfo({required this.version, required this.body, required this.downloadUrl, required this.assetName});
}
...
class UpdateService {
  ...
  Future<UpdateInfo?> checkForUpdate() async {
    ...
    try {
      ...
      final response = await _dio.get('/repos/$_kOwner/$_kRepo/releases/latest');
  ...
  Future<void> downloadAndInstall(UpdateInfo info) async {
    ...
    try {
      await _dio.download(info.downloadUrl, filePath,
        options: Options(headers: {'Accept': 'application/octet-stream'}),
        onReceiveProgress: (r, t) { if (t > 0) downloadProgress.value = r / t; },
      );
  ...
  Future<void> _installWindows(String zipPath) async {
    ...
    final script = 'Start-Sleep 2\nExpand-Archive -Force "$zipPath" "${dir.path}\\upd"\nCopy-Item "${dir.path}\\upd\\*" "$appDir" -Recurse -Force\nStart-Process "$exePath"';
    final f = File('${dir.path}\\update.ps1')..writeAsStringSync(script);
    await Process.start('powershell', ['-ExecutionPolicy','Bypass','-File',f.path], mode: ProcessStartMode.detached);
    exit(0);
  }

  Future<void> _installMacOS(String zipPath) async {
    final dir = await getTemporaryDirectory();
    final appBundle = File(Platform.resolvedExecutable).parent.parent.parent.path;
  ...
  Map<String, dynamic>? _findAssetForPlatform(List<dynamic> assets) {
    final suffix = Platform.isWindows ? '_windows.zip' : Platform.isMacOS ? '_macos.zip' : '_linux.tar.gz';
    for (final a in assets) {
      if ((a['name'] as String).endsWith(suffix)) return a as Map<String, dynamic>;
    }
    return null;
  }
  ...
  bool _isNewer(String latest, String current) {
    try {
      final l = _parse(latest); final c = _parse(current);
      for (int i = 0; i < 3; i++) { if (l[i] > c[i]) return true; if (l[i] < c[i]) return false; }
      return false;
    } catch (_) { return false; }
  }

  List<int> _parse(String v) => v.replaceFirst('v','').split('-').first.split('.').map((p) => int.tryParse(p) ?? 0).toList();
}
