...
Ограничение платформы: в Rlink у сторонних ботов нет Telegram‑InlineKeyboard.
«Кнопки» = нумерованное меню (ответ цифрой 1–8) + slash‑команды.
"""
...
Там же таблицы по командам **Lib** и краткий чеклист создания бота.""",
    ...
    (
        ...
        """Здесь нет графических кнопок как в Telegram: напишите **цифру 1–8** или команду **`/menu`**, **`/start`**.

Можно копировать варианты из блока меню — ответ придёт отдельным сообщением.""",
    ),
]
